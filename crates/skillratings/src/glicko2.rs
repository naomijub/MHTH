...
mod tests {
    ...
    fn test_equal_glicko2() {
        ...
        let (player1new, player2new) = glicko2(&player1, &player2, &Outcomes::SUCCESSFUL, &config);
    ...
    fn test_glicko2_rating_period() {
        ...
        let results = vec![
            ...
            (opponent_three, Outcomes::FAILURE),
        ];
    ...
    fn test_single_rp() {
        ...
        let (np, _) = glicko2(&player, &opponent, &Outcomes::SUCCESSFUL, &config);

        let rp = glicko2_rating_period(&player, &[(opponent, Outcomes::SUCCESSFUL)], &config);
