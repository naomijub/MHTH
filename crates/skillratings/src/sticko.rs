...
mod tests {
    ...
    fn test_sticko() {
        ...
        let (player1, _) = sticko(&player1, &opponent1, &Outcomes::SUCCESSFUL, &config);

        let (player1, _) = sticko(&player1, &opponent2, &Outcomes::FAILURE, &config);

        let (player1, _) = sticko(&player1, &opponent3, &Outcomes::FAILURE, &config);
    ...
    fn test_single_rp() {
        ...
        let (np, _) = sticko(&player, &opponent, &Outcomes::SUCCESSFUL, &config);

        let rp = sticko_rating_period(&player, &[(opponent, Outcomes::SUCCESSFUL, true)], &config);
    ...
    fn test_white_black() {
        ...
        let tournament2 = vec![
            (opponent1, Outcomes::SUCCESSFUL, false),
            (opponent2, Outcomes::FAILURE, false),
