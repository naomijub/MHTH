...
/// This means [`Outcomes::SUCCESSFUL`] is a win for `player_one` and [`Outcomes::FAILURE`] is a win for `player_two`.
///
...
pub fn trueskill(
    ...
    let rating_delta = match outcome {
        Outcomes::SUCCESSFUL | Outcomes::DRAW => player_one.rating - player_two.rating,
        Outcomes::FAILURE => player_two.rating - player_one.rating,
    };
    ...
    let (rank_multiplier1, rank_multiplier2) = match outcome {
        Outcomes::SUCCESSFUL | Outcomes::DRAW => (1.0, -1.0),
...
///     &player_one,
///     &vec![
///         (player_two, Outcomes::SUCCESSFUL),
///         (player_three, Outcomes::SUCCESSFUL),
///         (player_four, Outcomes::FAILURE),
///     ],
///     &TrueSkillConfig::new(),
/// );
///
...
pub fn trueskill_rating_period(
    ...
    for (opponent, result) in results {
        ...
        let rating_delta = match result {
            Outcomes::SUCCESSFUL | Outcomes::DRAW => player_rating - opponent.rating,
            Outcomes::FAILURE => opponent.rating - player_rating,
        };
        ...
        let rank_multiplier = match result {
            Outcomes::SUCCESSFUL | Outcomes::DRAW => 1.0,
            Outcomes::FAILURE => -1.0,
        };
...
pub fn trueskill_two_teams(
    ...
    let rating_delta = match outcome {
        Outcomes::SUCCESSFUL | Outcomes::DRAW => rating_one_sum - rating_two_sum,
...
///
/// assert!((new_one[0].rating - 25.622306739859756).abs() < f64::EPSILON);
/// assert!((new_one[1].rating - 30.012965086723046).abs() < f64::EPSILON);
/// assert!((new_one[2].rating - 21.3786357876259).abs() < f64::EPSILON);
///
/// assert!((new_two[0].rating - 28.246057397676058).abs() < f64::EPSILON);
/// assert!((new_two[1].rating - 41.09193213651812).abs() < f64::EPSILON);
/// assert!((new_two[2].rating - 20.064520412174183).abs() < f64::EPSILON);
///
/// assert!((new_three[0].rating - 21.131635862464194).abs() < f64::EPSILON);
/// assert!((new_three[1].rating - 29.25702408561156).abs() < f64::EPSILON);
/// assert!((new_three[2].rating - 16.953981169279245).abs() < f64::EPSILON);
/// ```
...
type TruncLayerFn = Box<dyn Fn(f64, f64, f64) -> f64>;
...
mod tests {
    ...
    fn test_teams() {
        ...
        let (team_one, _) = trueskill_two_teams(
            &[player_one],
            &[],
            &Outcomes::SUCCESSFUL,
            &TrueSkillConfig::new(),
    ...
    fn test_traits() {
        ...
        let (new_player_one, new_player_two) = RatingSystem::rate(
            &rating_system,
            &player_one,
            &player_two,
            &Outcomes::SUCCESSFUL,
        ...
        let new_player_one = RatingPeriodSystem::rate(
            &rating_period,
            &player_one,
            &[(player_two, Outcomes::SUCCESSFUL)],
        );
    ...
    fn test_unlikely_ffa() {
        ...
        assert!((results[3][0].rating - 3.577_783_039_440_317_4).abs() < f64::EPSILON);

        assert!((results[0][0].uncertainty - 4.453_979_220_480_199).abs() < f64::EPSILON);
        assert!((results[1][0].uncertainty - 1.871_855_882_391_709_3).abs() < f64::EPSILON);
        assert!((results[2][0].uncertainty - 0.083_922_196_135_183_59).abs() < f64::EPSILON);
        assert!((results[3][0].uncertainty - 1.197_926_990_096_512).abs() < f64::EPSILON);
    }
