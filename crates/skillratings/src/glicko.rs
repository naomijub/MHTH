...
///     (opponent2, Outcomes::FAILURE),
///     (opponent3, Outcomes::FAILURE),
/// ];
///
/// let config = GlickoConfig::new();
///
...
mod tests {
    ...
    fn test_glicko() {
        ...
        let (player1, _) = glicko(&player1, &opponent1, &Outcomes::SUCCESSFUL, &config);

        let (player1, _) = glicko(&player1, &opponent2, &Outcomes::FAILURE, &config);

        let (player1, _) = glicko(&player1, &opponent3, &Outcomes::FAILURE, &config);
    ...
    fn test_single_rp() {
        ...
        let (np, _) = glicko(&player, &opponent, &Outcomes::SUCCESSFUL, &config);

        let rp = glicko_rating_period(&player, &[(opponent, Outcomes::SUCCESSFUL)], &config);
